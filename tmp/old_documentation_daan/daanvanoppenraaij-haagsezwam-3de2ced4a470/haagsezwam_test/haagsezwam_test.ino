 ...
 */
int alarm_old = 0;
bool relay_1_old = false;
bool relay_2_old = false;
bool relay_air_in_old = false;
bool relay_air_out_old = true;
...
/**
  ...
  Serial.println("On ('y') or on-off ('n'):");
  ...
  Serial.println("Set the MIST timer (first repitition):");
  while (Serial.available() == 0);
  ...
  invoerMist[0] = incoming.toInt();

  Serial.println("Set the OFF timer (first repitition):");
//  Serial.println(Serial.available() );
  ...
  invoerUit[0] = incoming.toInt();

  Serial.println("Set the AIR timer (first repitition):");
  ...
  invoerLucht[0] = incoming.toInt();

//  Serial.println("Set the MIST timer (second repitition):");
//  while (Serial.available() == 0);
//        
//  while (Serial.available() > 0) 
//    incoming = Serial.readString();
...
//  Serial.println(incoming.toInt());
//
//  invoerMist[1] = incoming.toInt();
//
//  Serial.println("Set the OFF timer (second repitition):");
////  Serial.println(Serial.available() );
//  while (Serial.available() == 0);
//      
//  while (Serial.available() > 0) 
...
//  Serial.println(incoming.toInt());
//
//  invoerUit[1] = incoming.toInt();
//
//  Serial.println("Set the AIR timer (second repitition):");
//
//  while (Serial.available() == 0);
//      
//  while (Serial.available() > 0) 
...
//  Serial.println(incoming.toInt());
//
//  invoerLucht[1] = incoming.toInt();


  Serial.println("Type something to start");
  ...
  Alarm.timerOnce( 0, hello);
  Alarm.timerOnce( 1, hello);
         
  
}

void hello(){
  Serial.println("HELLLLLOOOOO");
}
...
void loop() {
 ...
 //------------------------------------------mist1-------------------------------------------------------------------------
  ...
  {
    Serial.println("Went into the initial loop");
    int timer = 0;
    startEenmaal = false;
    ...
    for( int j = 0; j < invoerPeriodes; j++)
    {
      for( int i = 0; i < invoerHerhaal[j]; i++)
      {
        // If not zero
        // if(invoerMist[j]){                          
        //   Alarm.timerOnce(timer * SEC, mistAanLuchtUit);    
        //   Alarm.timerRepeat((timer + totaleTijd) * SEC, mistAanLuchtUit); 
        // }

        Serial.println("Went into the double   loop");



        /**
        *   New code with on-off cycle for humididier
        */
...
//        if(invoerMist[j]){
//          Serial.println("Went into the if function");
//
//          // timerOnce does not trigger if timer = 0
//          if(!timer)
//            nieuwMistAanLuchtUit();
//          Alarm.timerOnce( timer * SEC, nieuwMistAanLuchtUit);
//          Alarm.timerRepeat( (timer + totaleTijd) * SEC, nieuwMistAanLuchtUit);
//        }
//        timer += invoerMist[j];
// 
//        if(invoerUit[j]){   
          // Turn off cycle
//          Alarm.timerOnce( timer * SEC, freeTimer );
//          Alarm.timerRepeat( (timer + totaleTijd) * SEC, freeTimer);

//          Alarm.timerOnce(timer * SEC, beideUit);    
//          Alarm.timerRepeat((timer + totaleTijd) * SEC, beideUit); 
//        }
...
//          Alarm.timerRepeat( (timer + totaleTijd) * SEC, freeTimer);
//
//          Alarm.timerOnce(timer * SEC, luchtAanMistUit);    
//          Alarm.timerRepeat((timer + totaleTijd) * SEC, luchtAanMistUit); 
//        }
  ...
  if(info ){
    if( check != now()){
      check = now();
      updateTimeScreen(now());  
    }
  }  
  if( relay_1_old != digitalRead(RELAY_1) || 
      relay_2_old != digitalRead(RELAY_2) || 
      relay_air_in_old != digitalRead(RELAY_AIR_IN) || 
      relay_air_out_old != digitalRead(RELAY_AIR_OUT) ||
      alarm_old != Alarm.count())
    {

//    Serial.print("\tTijd = ");
     
    relay_1_old = digitalRead(RELAY_1); 
    relay_2_old = digitalRead(RELAY_2);
    relay_air_in_old = digitalRead(RELAY_AIR_IN);
    relay_air_out_old = digitalRead(RELAY_AIR_OUT);
    alarm_old = Alarm.count();

    Serial.print("\ttijd = "); Serial.print(String(hour(now()))+":"+String(minute(now()))+ ":"+String(second(now())));
    Serial.print("\ttt = "); Serial.print(String(hour(now() - old_time))+":"+String(minute(now() - old_time))+ ":"+String(second(now() - old_time)));
    old_time = now();    
    Serial.print("\tAlarms = "); Serial.print(Alarm.count());
    Serial.print("\tR1 = "); Serial.print(digitalRead(RELAY_1));
    Serial.print("\tR2 = "); Serial.print(digitalRead(RELAY_2));
    Serial.print("\tAIR_IN = "); Serial.print(digitalRead(RELAY_AIR_IN ));
    Serial.print("\tAIR_OUT = "); Serial.println(digitalRead(RELAY_AIR_OUT));
  }
}
...
void nieuwMistAanLuchtUit(){
  Serial.println("Nieuw Mist Aan lucht uit");
  ...
  // Make sure the last cycle is stopped before the next stage is called. 
  // stopTime -= fanSec;
  // Alarm.timerOnce( stoptime, freeTimer);
}
...
void freeTimer(){
  ...
  for(int i=0; i < ID_SIZE; i++){
    ...
    // Alocate freed ID to a non-usable id (to not accidently free other timers when randomly called)
    id[i] = 999;
  }
  Serial.print("After ");
  Serial.println(Alarm.count());
  // id = NeverUsedID; // AlarmID that is never in use so that we don't accidentally free a used id
...
//  Serial.print("Lucht aan mist uit ");
//  Serial.println(Alarm.count());
